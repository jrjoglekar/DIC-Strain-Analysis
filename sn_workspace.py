...
all_notches_folder = '/Users/janhavijoglekar/Desktop/174h1050dirs'
...
for notch_name in os.listdir(all_notches_folder):
    ...
    if os.path.isdir(notch_path) and notch_name.startswith("notch_"):
        ...
        # Enter the pattern of the .txt file with the original filenames (current notch is a changing variable)
        original_names_text_file = f'174_H1050_cycles100to110_sorted_Notch_0{current_notch_num}_original_filenames.txt'

        # Enter the pattern of the sorted notch images subfolders (current notch is a changing variable)
        ...
        if os.path.isdir(results_folder_path):
            ...
            # Example: get all .txt files inside results
            file_pattern = os.path.join(results_folder_path, '*.txt')
...
# Next steps:
# Find a way to get the N_f value easily from the crack initiation cycle google sheets
# Generally find ways to make this more hands-off
# Overlay location of critical subset
# Identify strain evolution--how many cycles till steady state?
